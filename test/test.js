...
/**
 * Test seaching for a new created game
 */
describe('POST /api/games/seach', () => {
  let gameId = '';
  beforeEach(async () => {
    const data = {
      publisherId: '1234567890',
  ...
  it('respond with json created game', (done) => {
    request(app)
      ...
      .end((err, result) => {
        ...
        assert.ok(result.body[0].name.includes('Test'));
        assert.ok(result.body[0].platform.includes('ios'));
        return done();
      });
  });
});
